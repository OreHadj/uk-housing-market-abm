...
export function Report2Page({ presentationControls, queueControls, execution, canWrite }: { presentationControls: ReactNode; queueControls?: ReactNode; execution?: ReportExecutionState; canWrite: boolean }) {
  ...
  return <section className="report2" aria-labelledby="r2-title">
    ...
    {!loading && payload && <>
      ...
      {(primary?.status === 'partial' || comparison?.status === 'partial') && <p className="r2-notice">Output is partial. Figures cover recorded months only.</p>}
      ...
      <div className="r2-dashboard">
        ...
        <div className="r2-main-grid">
          ...
          <article className="r2-card r2-market" data-guided-target="policy-market">
            ...
            {trend ? <div id="r2-market-chart" className="r2-market-chart-wrap" role="img" aria-label={`${MARKET.find((item) => item.id === marketId)?.label} · ${[showMarketSelected && 'selected run', hasComparison && showMarketBaseline && 'baseline'].filter(Boolean).join(' and ') || 'both runs hidden'}. Monthly observations are summarised below.`}><Report2MarketChart option={trend} showSelected={showMarketSelected} showBaseline={hasComparison && showMarketBaseline} />{!showMarketSelected && !(hasComparison && showMarketBaseline) && <p className="r2-market-hidden-hint">Use the legend dots to show a run.</p>}</div> : <Empty>This indicator has no recorded values in the selected window.</Empty>}
          ...
          <article className="r2-card r2-risk">
            ...
            {!lendingLoading && !lendingError && transactionRecordingDisabled && <p className="r2-footnote r2-recording-help"><strong>To enable recording,</strong> go to Experiments → Policy scenarios → Technical details → Additional data exports → <strong>Record transactions</strong>. Set <strong>Start recording at month</strong> no later than your analysis window’s start, then start a new run. For comparisons, enable baseline recording too.</p>}
            <p className="r2-footnote">{hasComparison ? `Selected run (${riskDefinition(primaryLending)}). Baseline (${riskDefinition(comparisonLending)}).` : `${riskDefinition(primaryLending)}.`} Only recorded mortgages are included. Cash purchases are excluded.{thresholdsDiffer && ' The thresholds differ, so these shares use different risk definitions.'}</p>
          ...
          <article className="r2-card r2-rental" data-guided-target="policy-rental">
            ...
            <div className="r2-rental-body">
              <div className="r2-tenure">
                {tenure ? <div role="img" aria-label={`The selected run has ${number(ownership, '%')} owner-occupiers and ${number(rentingShare, '%')} private renters. Remaining households shown separately.`}><EChart option={tenure} className="r2-tenure-chart" /></div> : <Empty>Tenure shares are not available.</Empty>}
